...
let rec run_machine machine =
  ...
  let transitions = List.assoc machine.state machine.description.transitions in
  ...
  in
  let new_tape = Printf.sprintf "%s%s%s" before rule.write after in
  let new_pos =
  ...
  if List.mem new_state machine.description.finals then ()
  else
    run_machine
